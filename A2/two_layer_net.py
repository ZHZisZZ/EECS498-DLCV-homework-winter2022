...
def nn_forward_backward(params, X, y=None, reg=0.0):
    ...
    ############################################################################
    logits = scores - scores.max(-1, True).values
    log_prob = logits - logits.logsumexp(-1, True)
    loss = -log_prob[range(N), y].mean()
    ...
    ###########################################################################
    prob = log_prob.exp()
    dldz = (prob - torch.nn.functional.one_hot(y, prob.shape[1])) / N  # (N, C)
    grads['b2'] = dldz.sum(0)                           # (C, )
    grads['W2'] = torch.matmul(h1.T, dldz)  + reg*2*W2  # (H, N) * (N, C) -> (H, C)
    dldh = torch.matmul(dldz, W2.T) * (h1 > 0)          # (N, C) * (C, H) -> (N, H)
    grads['b1'] = dldh.sum(0)
    grads['W1'] = torch.matmul(X.T, dldh) + reg*2*W1    # (D, N) * (N, H) -> (D, H)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def find_best_net(data_dict, get_param_set_fn):
  ...
  for lr, hs, rs, lrd in product(get_param_set_fn()):
    i += 1
    print("""Training two layer net %d with learning_rate=%e, 
          hidden_size=%e, reg=%e and learning_rate_decay=%e"""
          % (i, lr, hs, rs, lrd))
    ...
    if val_acc > best_val_acc:
      ...
      best_net = net
      best_stat = stat
    results[(lr, hs, rs, lrd)] = val_acc

  for lr, hs, rs, lrd in sorted(results):
    val_acc = results[(lr, hs, rs, lrd)]
    print('lr %e hs %e reg %e lrd %e val accuracy: %f' 
          % (lr, hs, rs, lrd, val_acc))
