...
class Conv(object):
  ...
  def forward(x, w, b, conv_param):
    ...
    padded_x = torch.nn.functional.pad(x, 4*(pad,))
    windows = []
    for i in range(H_out):
      for j in range(W_out):
        ...
        windows.append(window)                                  # List[(N, C, HH, WW)] (H_out*W_out)
    squeezed_x = torch.stack(windows, dim=-1)                   # (N, C, HH, WW, H_out*W_out)
    squeezed_x = squeezed_x.view(N, -1, H_out*W_out)            # (N, C*HH*WW, H_out*W_out)

    squeezed_w = w.view(F, -1)                                  # (F, C*HH*WW)

    squeezed_out = torch.matmul(squeezed_w, 
                                squeezed_x) + b.unsqueeze(-1)   # (N, F, H_out*W_out)

    out = squeezed_out.view(N, F, H_out, W_out)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, squeezed_x, w, b, conv_param)
    return out, cache
  ...
  def backward(dout, cache):
    ...
    #############################################################################
    x, squeezed_x, w, b, conv_param = cache                      # (N, C*HH*WW, H_out*W_out)
    N, C, H, W = x.shape
    F, C, HH, WW = w.shape
    stride, pad = conv_param['stride'], conv_param['pad']
    H_out = 1 + (H + 2 * pad - HH) // stride
    ...
    squeezed_dout = dout.view(N, F, -1)                           #  (N, F, H_out*W_out)
    db = squeezed_dout.sum((0, 2))
    squeezed_dw = torch.matmul(squeezed_dout, 
                               squeezed_x.permute(0,2,1)).sum(0)  #  (N, F, H_out*W_out) * (N, H_out*W_out, C*HH*WW) -> (F, C*HH*WW)
    ...
    squeezed_dx = squeezed_dx.view(N, C, HH, WW, -1)
    ...
    for i in range(H_out):
      for j in range(W_out):
        padded_dx[:, :, 
            i*stride:i*stride+HH, 
            j*stride:j*stride+WW] += squeezed_dx[..., H_out*i+j]
...
class MaxPool(object):
  ...
  def forward(x, pool_param):
    ...
    for i in range(H_out):
      for j in range(W_out):
        ...
        windows.append(window)                            #  List[(N, C, HH, WW)] (H_out*W_out)
    squeezed_x = torch.stack(windows, dim=-1)             #  (N, C, HH, WW, H_out*W_out)
    out = squeezed_x.amax(dim=(2,3)).view(N, C, H_out, W_out)
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    cache = (x, squeezed_x, pool_param)
    return out, cache
  ...
  def backward(dout, cache):
    ...
    #############################################################################
    x, squeezed_x, pool_param = cache
    N, C, H, W = x.shape
    pool_height, pool_width, stride = pool_param['pool_height'], pool_param['pool_width'], pool_param['stride']
    H_out = 1 + (H - pool_height) // stride
    W_out = 1 + (W - pool_width ) // stride

    squeezed_dout = dout.view(N, C, -1)
    mask = (squeezed_x == squeezed_x.amax((2,3), True))
    squeezed_dx = squeezed_dout.unsqueeze(2).unsqueeze(2) * mask
    dx = x.new_zeros(x.shape)
    for i in range(H_out):
      for j in range(W_out):
        dx[:, :, 
           i*stride:i*stride+pool_height, 
           j*stride:j*stride+pool_width] += squeezed_dx[..., H_out*i+j]
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    return dx
...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    ############################################################################
    C, H, W = input_dims
    flatten_dim = num_filters * input_dims[1] * input_dims[2] // 4
    self.params['W1'] = weight_scale * torch.randn(num_filters, C, 
  ...
  def loss(self, X, y=None):
    ...
    ############################################################################
    caches = [None]*3
    hidden, caches[0] = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
    hidden, caches[1] = Linear_ReLU.forward(hidden, W2, b2)
    scores, caches[2] = Linear.forward(hidden, W3, b3)
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    dh, grads['W3'], grads['b3'] = Linear.backward(dh, caches[2])
    dh, grads['W2'], grads['b2'] = Linear_ReLU.backward(dh, caches[1])
    __, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dh, caches[0])
    for i in range(3):
...
class DeepConvNet(object):
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    for i, out_channel in enumerate(num_filters):
      self.params[f'b{i+1}'] = torch.zeros(out_channel)
      if weight_initializer:
        self.params[f'W{i+1}'] = weight_initializer(
            in_channel, out_channel, 3, True, device, dtype)
      else:
        self.params[f'W{i+1}'] = weight_scale * torch.randn(out_channel, in_channel, 3, 3)
      if batchnorm:
        self.params[f'gamma{i+1}'] = torch.ones(out_channel)
        self.params[f'beta{i+1}'] = torch.zeros(out_channel)
      in_channel = out_channel

    # linear
    flattened_dim = out_channel * input_dims[1] * input_dims[2] // 4**len(max_pools)
    self.params[f'b{self.num_layers}'] = torch.zeros(num_classes)
    if weight_initializer:
      self.params[f'W{self.num_layers}'] = weight_initializer(
        flattened_dim, num_classes, None, False, device, dtype)
    else:
      self.params[f'W{self.num_layers}'] = weight_scale * torch.randn(flattened_dim, num_classes)

    for name, param in self.params.items():
      self.params[name] = param.type(dtype).to(device)
    # TODO: batch norm parameters
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
      ...
      if i+1 == self.num_layers:
        ...
        # Conv_BatchNorm_ReLU.forward is modified for unified API and backward compatilibity
        layer = Conv_BatchNorm_ReLU_Pool if i in self.max_pools else Conv_BatchNorm_ReLU
        hidden, cache = layer.forward(hidden, W, b, gamma, beta, 
                                      conv_param, self.bn_params[i], pool_param)
      else:
        # Conv_ReLU.forward is modified for unified API and backward compatilibity
        layer = Conv_ReLU_Pool if i in self.max_pools else Conv_ReLU
    ...
    for i, (cache, layer) in reversed(list(enumerate(caches))):
      if self.batchnorm:
        dh, grads[f'W{i+1}'], grads[f'b{i+1}'], \
        grads[f'gamma{i+1}'], grads[f'beta{i+1}'] = layer.backward(dh, cache) 
      else:
        dh, grads[f'W{i+1}'], grads[f'b{i+1}'] = layer.backward(dh, cache)
    # regularization
    for i in range(self.num_layers):
      loss += self.reg * self.params[f'W{i+1}'].pow(2).sum()
      grads[f'W{i+1}'] += self.reg * 2 * self.params[f'W{i+1}']
...
class BatchNorm(object):
  ...
  def backward(dout, cache):
    ...
    ###########################################################################
    mode, x, t, gamma, std = cache
    dt = dout * gamma
    dx = dt / std   # x
    dbeta = dout.sum(0)
    dgamma = (dout * t).sum(0)
    if mode == 'train':
      dx -= (dt / std).mean(0) # -mean
      mean = x.mean(0)
      a = (dt * (x - mean) / std**2 / 2 / std).mean(0) * 2 * (x-mean)
      dx -= a - a.mean(0)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
class SpatialBatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    x = x.permute((0, 2, 3, 1)).view(-1, C)  # (N*H*W, C)
    out, cache = BatchNorm.forward(x, gamma, beta, bn_param)
    out = out.shape(N, C, H, W)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
  ...
  def backward(dout, cache):
    ...
    ###########################################################################
    N, C, H, W = dout.shape
    dout = dout.permute((0, 2, 3, 1)).view(-1, C)  # (N*H*W, C)
...
class Conv_ReLU(object):
  ...
  @staticmethod
  def forward(x, w, b, conv_param, *args):
    """
...
class Conv_BatchNorm_ReLU(object):
  ...
  @staticmethod
  def forward(x, w, b, gamma, beta, conv_param, bn_param, *args):
    a, conv_cache = FastConv.forward(x, w, b, conv_param)
