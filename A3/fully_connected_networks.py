...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    ###########################################################################
    hidden, cache1 = Linear_ReLU.forward(X, self.params['W1'], self.params['b1'])
    scores, cache2 = Linear.forward(hidden, self.params['W2'], self.params['b2'])
    ###########################################################################
    #                            END OF YOUR CODE                             #
    ###########################################################################
    ...
    dh, grads['W2'], grads['b2'] = Linear.backward(ds, cache2)
    _,  grads['W1'], grads['b1'] = Linear_ReLU.backward(dh, cache1)
    # regularization
    loss += self.reg * (self.params['W1'].pow(2).sum() + 
                        self.params['W2'].pow(2).sum())
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    - input_dim: An integer giving the size of the input.
    - num_classes: An intehiddeger giving the number of classes to classify.
    ...
    ############################################################################
    # input dimension and output dimension for each layer
    in_dim, out_dim = input_dim, None
    for i, out_dim in enumerate(hidden_dims + [num_classes]):
      self.params[f'W{i+1}'] = weight_scale * torch.randn(in_dim, out_dim)
    ...
    assert len(self.params) == self.num_layers * 2
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers):
      ...
      if i+1 == self.num_layers:  # output layer
        ...
        hidden, cache1 = Linear_ReLU.forward(hidden, W, b)
        hidden, cache2 = Dropout.forward(hidden, self.dropout_param)
        cache = (cache1, cache2)
      else:
        hidden, cache  = Linear_ReLU.forward(hidden, W, b)
    ...
    for i , cache in reversed(list(enumerate(caches))):
      if i+1 == self.num_layers:  # output layer
        dh, grads[f'W{i+1}'], grads[f'b{i+1}'] = Linear.backward(dh, cache)
      elif self.use_dropout:
        dh = Dropout.backward(dh, cache[1])
        dh, grads[f'W{i+1}'], grads[f'b{i+1}'] = Linear_ReLU.backward(dh, cache[0])
      else:
        dh, grads[f'W{i+1}'], grads[f'b{i+1}'] = Linear_ReLU.backward(dh, cache)
    # regularization
    for i in range(self.num_layers):
...
def rmsprop(w, dw, config=None):
  ...
  cache = decay_rate * cache + (1-decay_rate) * dw.pow(2)
  next_w = w - learning_rate * dw / (cache + epsilon)
  config['cache'] = cache
  ###########################################################################
  #                             END OF YOUR CODE                            #
  ###########################################################################
...
def adam(w, dw, config=None):
  ...
  v = beta2 * v + (1 - beta2) * dw.pow(2)
  m_u = m / (1 - beta1 ** t)
  v_u = v / (1 - beta2 ** t)
  next_w = w - learning_rate * m_u / (v_u.sqrt() + epsilon)
...
class Dropout(object):
  ...
  def forward(x, dropout_param):
    ...
    if mode == 'train':
      ...
      ###########################################################################
      p = 1-p
      mask = ((torch.rand(x.shape) < p) / p).to(x.device) 
      out = x * mask
      ###########################################################################
      #                             END OF YOUR CODE                            #
      ###########################################################################
